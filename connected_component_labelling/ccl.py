...
# Choose highest label region  with getting rid of noise  
def handdetect(label_image, origin_image):
	#print('The count of 1 is:', label_image[1].count(1))
	count = 0
	# Detect region larger than 50 pixels
	for i in range (1,label_image.max(),1):		
		for y, row in enumerate(label_image):
			for x, pixel in enumerate(row):	
				if label_image[y,x] == i:						
					count += 1
		if count > 50:
			break;
		else: 
	...
	for y, row in enumerate(label_image):
		for x, pixel in enumerate(row):		
			if label_image[y,x] != i:
				origin_image[y,x] = 0
			elif label_image[y,x] == i:
				origin_image[y,x] = 255
				
	#print_image(label_image)
	return origin_image	
...
if __name__ == "__main__":
	import sys
	if(len(sys.argv) > 1): # if choose connectivity
		connectivity_type = int(sys.argv[1])
	else:
		connectivity_type = CONNECTIVITY_8 # default
	#datasetlist = ["b", "c","d","e","f","g","h","i","j"]
	datasetlist = [ "test", "test2"]
	for i in range(len(datasetlist)):	
		filename=os.listdir(r'/home/vssdeep/Desktop/chi_yu/preprocessing/connected_component_labelling/Dataset/'+datasetlist[i])	
		filenumber=np.size(filename)
		
		for k in range(0, filenumber):
			image = cv2.imread('Dataset/'+datasetlist[i]+'/'+filename[k])
			gray_image = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
			gray_image_origin = gray_image				
			bool_image = image_to_2d_bool_array(gray_image)
			ccl = connected_component_labelling(bool_image, connectivity_type)
			output = handdetect(ccl,gray_image_origin)
			#cv2.imwrite('123.png',gray_image_origin)
			cv2.imwrite('crop/'+datasetlist[i]+'/crop_'+filename[k], output)
			#cv2.imwrite('456.png', output)
# Run in Python
